...
class FRM(nn.Module):
    ...
    def forward(self, x_pvt, x_eff):
        ...
        #SSU
        bs = x_eff.shape[0]
        eff_pool = self.gap_1(x_eff) + self.max_1(x_eff)
        eff_pool = eff_pool.view(bs, -1)
        pvt_pool = self.gap_2(x_pvt) + self.max_2(x_pvt)
        pvt_pool = pvt_pool.view(bs, -1)
        feat = torch.cat((eff_pool, pvt_pool), dim=1)
        gate_co = self.fc(feat).mean(dim=-1).view(bs, 1, 1, 1)
        ssu_out = self.ssu(x_pvt, x_eff, gate_co)
        #FCU
